...
namespace Travel.Data.Repositories
{
    public class StatisticRes : IStatistic
    {
        ...
        public StatisticRes(TravelContext db, NotificationContext dbNotyf, ITourBooking tourBooking)
        {
            _db = db;
            _dbNotyf = dbNotyf;
        ...
        {
            List<TourBooking> listTourBookingFinished = new();
            ...
            try
            {
                ...
                if (dateSaveMax == null || dateSaveMax.DateSave < input)
                {
                    ...
                    {
                        var listIdScheduleFinished = await _tourBooking.CallServiceGetListIdScheduleFinished();


                        foreach (var item in listIdScheduleFinished)
                        {
                            var ls = await (from tbk in _db.TourBookings.AsNoTracking()
                                            where tbk.Status == (int)Enums.StatusBooking.Finished
                                            && tbk.ScheduleId == item
                                            select tbk).ToListAsync();
                            listTourBookingFinished.AddRange(ls);
                        }
                        ...
                        foreach (var item in listGroupingTourbooking)
                        {
                            ...
                            var tour = await _tourBooking.CallServiceGetTourByIdSchedule(item.Key);


                            var schedule1 = await _tourBooking.CallServiceGetSchedule(item.Key);
                            var costTour = schedule1.TotalCostTour;
        ...
        public async Task<bool> SaveReportWeek()
        {
            try
            {
